...
I'll be plotting a weighted mean by population (following up on some plotting I did in the last assignment, where I calculated the mean by country). To get a sense of what we're working with, here is a graph comparing life expectancy and population:
...
pop_growth %>% 
  ...
  ggtitle("Percent population growth of Asian countries", subtitle="1952-1967") + 
...
gapminder %>% 
  ...
  ggplot(aes(x=year, y=avggdp, color=continent)) + geom_line()
```

We can see a spike in Asia's average GDP per capita, followed by a slight fall. Since the plot is weighted by country, we know to look for one with a smaller population.
...
datatable(gdp_asia)
gdp_asia %>% 
  ggplot(aes(x=year,y=gdpPercap, group=country, color=country=="Kuwait")) + geom_line()
```
