...
We can see that the durations of songs varies greatly, with some of the longest songs being around 2000 seconds. We can organise the decades in order of greatest to smallest maximum duration (per decade).
...
Here are two bar graphs of the number of songs listed for each decade. The second graph has been reordered by number of entries per decade.
...
The text doesn't change size with the plot; that needs to be changed explicitly when creating the plot, using `theme(axis.text = element_text(size = [some_number_here]))`.
